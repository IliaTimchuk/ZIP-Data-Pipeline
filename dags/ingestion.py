...
def ingest_from_s3():
    ...
    def load_sources_config(**context) -> list[dict]:
        ...
        valid_sources, errors = get_dag_sources(
            dag_id,
            conf.SOURCES_YAML_PATH,
            conf.INGEST_FROM_S3_SOURCE_SCHEMES,
            date=ds,
        )
    ...
    def extract_files(bucket, key, aws_conn_id):
        """
        ...
        def stream_file(
            source_bucket: str, source_key: str, source_conn_id: str
        ) -> None:
            """
            Streams a single file from a source S3 bucket into the internal raw
            data bucket, in chunks without writing it to local disk first.
            """
            ...
            dest_key = f"{source_bucket}/{source_key}"
        ...
        wait_for_file >> stream_file(bucket, key, aws_conn_id)
